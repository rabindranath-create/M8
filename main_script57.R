...
output_dir <- file.path(getwd(), "outputs/script57")
...
for(i in 1:6){
  ...
  bgpp_1 <- read.csv(paste0("CodeRD/code/bgpp", i, "/bgno57.csv"), header = FALSE, row.names = NULL)
  ...
  for(k in c(20, 30, 40, 50, 60)){
    ...
    for (j in 1:19) {
      ...
      path <- paste0("CodeRD/code/obstpp", j, "/obstpp", j, "_", k, "_57.csv")
